...
flatpickr('#pd', {
  ...
  onChange([d]) {
    const nx = new Date(d); nx.setDate(nx.getDate() + 3);
    rp.setDate(nx); rp.set('minDate', d);
  }
});
const rp = flatpickr('#rd', {
  minDate: 'today', dateFormat: 'D, d M Y',
  defaultDate: new Date(today.getTime() + 3 * 864e5)
});
...
function handleSearch() {
  ...
  setTimeout(() => {
    document.querySelectorAll('.car-card:not(.hidden)').forEach(c => {
      ...
      c.style.animation = 'fu .38s ease forwards';
    });
  }, 600);
}
function shake(id) {
  ...
  el.style.animation = 'shk .38s ease'; el.focus();
}
...
/* ── Scroll parallax — hero car drifts slightly right ──── */
const heroCar = document.getElementById('heroCar');
let rafPending = false;
window.addEventListener('scroll', () => {
  if (!heroCar || rafPending) return;
  rafPending = true;
  requestAnimationFrame(() => {
    const s = Math.min(window.scrollY, 500);
    heroCar.style.transform = `translateX(${s * 0.12}px)`;
    rafPending = false;
  });
});
